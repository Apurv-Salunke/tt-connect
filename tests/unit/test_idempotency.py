...
def test_tag_auto_generated():
    req = _req()
    assert req.tag
    assert len(req.tag) == 36  # full UUID4 with dashes


def test_each_request_gets_unique_key():
    keys = {_req().tag for _ in range(20)}
    assert len(keys) == 20


def test_user_can_set_explicit_key():
...
def test_zerodha_tag_included_in_params():
    req = _req()
    params = ZerodhaTransformer.to_order_params("2885", "RELIANCE", "NSE", req)
    assert "tag" in params


def test_zerodha_tag_max_20_chars():
    req = _req()
    params = ZerodhaTransformer.to_order_params("2885", "RELIANCE", "NSE", req)
    assert len(params["tag"]) <= 20


def test_zerodha_tag_alphanumeric_only():
    req = _req()
    params = ZerodhaTransformer.to_order_params("2885", "RELIANCE", "NSE", req)
    assert params["tag"].isalnum()


def test_zerodha_explicit_key_used_as_tag():
...
def test_zerodha_same_request_same_tag_on_retry():
